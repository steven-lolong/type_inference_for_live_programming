 ...
 * @fileoverview Expression List Operator - Head
 */
...
Blockly.Blocks["BlockOperatorListHead"] = {
  init: function () {
    this.syntaxType = "exp";
    ...
    this.setTooltip("Primitive list operator - head");
    this.setHelpUrl("");
    ...
    if (expBlock) {
      if (expBlock.termType.key == "list") {
        if (expBlock.termValue !== undefined) {
          let new_val = expBlock.termValue[0];
  ...
  inferType() {
    ...
    if (expBlock) {
      if (expBlock.termType.key == "list") {
        this.termType = expBlock.termType.value;
      }
    }
  },
